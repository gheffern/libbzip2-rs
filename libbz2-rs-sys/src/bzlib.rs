...
/// Drains repeated byte runs (>= 4 bytes) using vectorized memset and Slice-by-4 parallel CRC.
///
/// # Safety
///
/// Caller must ensure that `cs_next_out` points to a valid writable buffer of at least
/// `*cs_avail_out` bytes.
#[cold]
#[inline(never)]
unsafe fn drain_rle_bulk_cold(
    cs_next_out: &mut *mut c_char,
    cs_avail_out: &mut c_uint,
